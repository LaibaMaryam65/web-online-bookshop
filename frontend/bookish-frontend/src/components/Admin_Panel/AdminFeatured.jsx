...
export default function AdminFeatured() {
 ...
 async function handleAdd(book) {
  try {
    ...
    const added = await addBookToFeatured(featuredBook);
    if (added) loadFeatured();
  } catch (err) {
    alert("Failed to add book to Featured");
    console.error(err);
  }
}
  async function handleRemove(book) {
    try {
      ...
      await deleteFeaturedBook(book.id.toString());
      loadFeatured();
    } catch (err) {
      alert("Failed to remove book from Featured");
      console.error(err);
    }
  }
...
<div>
  ...
  <h2 className="text-center mb-4">Manage Featured Books</h2>
  ...
  <div className="row">
    ...
    <div className="col-lg-6 mb-4">
      <h3 className="mb-3 border-bottom pb-1">All Featured Books</h3>
